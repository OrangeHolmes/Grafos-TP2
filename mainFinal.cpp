...
vector<tuple<Grafo::vertice, Grafo::vertice, double>> Kruskal(Grafo g) {
    priority_queue<int, std::vector<int>, std::greater<int> >colaDePrioridadAristas;
    vector<tuple<Grafo::vertice, Grafo::vertice, double>> listaAristas;
    vector<tuple<Grafo::vertice, Grafo::vertice, double>> listaAristasCostoMinimo;
    Conjunto<Grafo::vertice>conjuntodeConjuntos;
    unordered_set<Grafo::vertice> verticesRecorridos;
    auto vertice1 = g.primerVertice();
    while (g.verticeValido(vertice1)) {
        verticesRecorridos.insert(vertice1); //
        conjuntodeConjuntos.agregar(vertice1);
        auto vAdyacente = g.primerVerticeAdyacente(vertice1);
        while (g.verticeValido(vAdyacente)) {
            if (!verticesRecorridos.count(vAdyacente)) { //
                tuple<Grafo::vertice, Grafo::vertice, double> arista(vertice1, vAdyacente, g.peso(vertice1, vAdyacente));
                colaDePrioridadAristas.push(g.peso(vertice1, vAdyacente));
                listaAristas.push_back(arista); 
            }
            vAdyacente = g.siguienteVerticeAdyacente(vertice1, vAdyacente);
        }
        vertice1 = g.siguienteVertice(vertice1);
    }
    ...
    int i;
    tuple<Grafo::vertice, Grafo::vertice, double> aristaPrioridad;
    int id1 = -1;
    int id2 = -1;
    while (cantAristas < numVertices - 1){
        if (!colaDePrioridadAristas.empty()) {
            pesoMenor = colaDePrioridadAristas.top();
            colaDePrioridadAristas.pop();
            i = 0;
            while (!aristaEncontrada) {
                if (pesoMenor == get<2>(listaAristas[i])) {
                    aristaPrioridad = listaAristas[i];
                    listaAristas.erase(listaAristas.begin() + i);
                    aristaEncontrada = true;
                }
				++i;
            }
			aristaEncontrada = false;
            id1 = conjuntodeConjuntos.aQueConjuntoPertenece(get<0>(aristaPrioridad));
            id2 = conjuntodeConjuntos.aQueConjuntoPertenece(get<1>(aristaPrioridad));
            if (id1 != id2) {
                ++cantAristas;
                conjuntodeConjuntos.unir(get<1>(aristaPrioridad), get<1>(aristaPrioridad));
                listaAristasCostoMinimo.push_back(aristaPrioridad);
            }
        }

    }
    cout << "\nKruskal(Aristas del Arbol Abarcador de Costo Minimo):\n";
    for (int i = 0; i < listaAristasCostoMinimo.size(); ++i) {
            cout << g.getEtiqueta(get<0>(listaAristasCostoMinimo[i])) << "-------" << get<2>(listaAristasCostoMinimo[i]) << "---------" << g.getEtiqueta(get<1>(listaAristasCostoMinimo[i])) << endl;
    }
    return listaAristasCostoMinimo;
}
